...
import { buscarEvidenciasAction, anexarEvidenciaAction } from "@/actions/coleta/gestor-coleta-actions";
...
export default function SheetDetalhesColeta({ coleta, open, onOpenChange }: SheetDetalhesProps) {
  ...
  useEffect(() => {
    if (open && coleta) {
      ...
      setError("");
      setIsEditing(false);
      if (coleta) setEditPeso(String(coleta.peso_kg || ""));
    }
  }, [open, coleta]);

  async function handleUpload(e: React.FormEvent) {
    e.preventDefault();
    if (!foto || !coleta) return;
  ...
  async function handleSaveEdit() {
    ...
    if (res.success) {
      setIsEditing(false);
      onOpenChange(false); // fecha o drawer pra forçar o refresh externo da lista, ou recarrega.
    } else {
        ...
        <div className="mt-6 flex flex-col gap-4">
          <div className="bg-muted/50 p-4 rounded-lg border">
            ...
            <div className="grid grid-cols-2 gap-y-2 text-sm">
              <span className="font-semibold text-muted-foreground">ID do Sistema:</span>
              <span className="truncate">{coleta.id_microservico}</span>
              
              <span className="font-semibold text-muted-foreground">Inscrição Imóvel:</span>
              ...
              {isEditing ? (
                ...
                <span className="font-bold text-foreground">{coleta.peso_kg} kg</span>
              )}
              
              <span className="font-semibold text-muted-foreground">Pontuação:</span>
              <span className="font-bold text-emerald-600">{coleta.pontuacao} pts</span>
              
              <span className="font-semibold text-muted-foreground">Origem:</span>
              <span>{coleta.registrado_por ? "Lançamento Manual" : "App de Campo"}</span>
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            {loadingEvidencias ? (
              ...
              <div className="flex flex-col gap-3">
                {evidencias.map((ev) => (
                  <div key={ev.id} className="border rounded-md overflow-hidden relative group">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={ev.arquivo_url} alt={ev.descricao || "Evidência"} className="w-full h-auto object-cover max-h-[250px]" />
                    {ev.descricao && (
